# scrape_bop_tenant.py
#
# Script to scrape telemetry (usage stats) of a specific tenant out of BOP.
#
...
#
# A separate script, produce_xls.py, is used to convert this json data into an .XLS file
...
def get_tenants_list(driver, tenant_type, tenant_domain):
...
def print_usage():
    print("Usage: scrape_bop_tenant.py <bop_url> [PAID|LITE] <tenant_domain>")
    print()
    print("Example: scrape_bop_tenant.py https://prod-bop.securiti.xyz PAID securiti.ai")
...
if __name__ == "__main__":
    ...
    tenant_domain = sys.argv[3]

    tenants = get_tenants_list(driver, tenant_type, tenant_domain)
    ...
    with open(bop_instance + " - " + tenant_domain + " - Tenant Stats - " + today + ".json", 'w') as outfile:
